...
def report14(data, save_path):
    ...
    for i in scatter_points:
        ...
        plt.annotate(
        f'{round(ReleaseSpeed[i], 1)}, {PlayResult[i]}',
    ...
    plt.title(f'{len(scatter_points)} Hits allowed', fontsize=16)
  ...
  # save image to folder
    plt.savefig(get_path(save_path, 'report14'))
    plt.close()